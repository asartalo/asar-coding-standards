...
use Squiz_Sniffs_Commenting_FunctionCommentSniff as FunctionCommentSniff;
 ...
 */
class Asar_Sniffs_Commenting_FunctionCommentSniff extends FunctionCommentSniff {

    protected $exemptedMethods = array(
        '__construct'
    );

    public function isExempted($methodName) {
        return in_array($methodName, $this->exemptedMethods);
    }
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr) {
        ...
        $funcName = $tokens[$stackPtr + 2]['content'];
        ...
        if ($code === T_COMMENT) {
            ...
            return;
        } else if ($code !== T_DOC_COMMENT && !$this->isExempted($funcName)) {
            $error = 'Missing function doc comment';
            $phpcsFile->addError($error, $stackPtr, 'Missing');
            return;
        }
        ...
        if (trim($short) === '' && !$this->isExempted($funcName)) {
        ...
        if (count($params) > 1) {
            ...
            if ($newlineSpan !== 2 && trim($comment->getShortComment()) !== '') {
        ...
        if (trim($comment->getShortComment()) !== '') {
